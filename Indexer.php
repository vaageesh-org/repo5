...
class Mage_CatalogIndex_Model_Resource_Indexer extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function stray_semicolon($x) {
        if ($x); { // STRAY_SEMICOLON here
            ++$x;
        }
    }
    public function unreachable($cond) {
        if($cond) {
            doWork();
        }
        return;
    ...
    public function forward_null($fn, $arg) {
        if ($fn != NULL) {
            something();
        }
    ...
    public function identical_branches_to_fix($a, $b, $op) {
